...
export const addVideos = videos => dispatch => {
  window.api.request('videos:added', videos);
  window.api.response('metadata:complete', (event, videosWithData) => {
...
export const convertVideos = videos => dispatch => {
  window.api.request('conversion:start', videos);

  window.api.response('conversion:end', (event, { video, outputPath }) => {
    dispatch({ type: VIDEO_COMPLETE, payload: { ...video, outputPath } });
  });

  window.api.response('conversion:progress', (event, { video, timemark }) => {
...
export const showInFolder = outputPath => dispatch => {
  window.api.request('folder:open', outputPath);
};
